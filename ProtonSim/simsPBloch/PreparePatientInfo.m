...
Size_Spread_FE_Variability_Rand = randn(1)*Size_Spread_FE_Variability;
mybeta = 0.0916947 + 0.000733*FE + Size_Spread_FE_Variability_Rand;   % scaling or spread parameter

%% if params become negative, use mean
if (mybeta < 0)
    Size_Spread_FE_Variability_Rand = 0;
    mybeta = 0.0916947 + 0.000733*FE;
end
...
Anisotropy_Shape_FE_Variability_Rand = randn(1)*Anisotropy_Shape_FE_Variability;
shapeEst = 1.024473 + 0.0178712*FE + Anisotropy_Shape_FE_Variability_Rand;

Anisotropy_Spread_FE_Variability_Rand = randn(1)*Anisotropy_Spread_FE_Variability;
scaleEst = exp(-3.270029 + 0.3448803*log(FE)) + Anisotropy_Spread_FE_Variability_Rand;

ampEst = 0.0754587 + 0.0039904*FE;

%% if params become negative, use mean
if (shapeEst < 0)
    Anisotropy_Shape_FE_Variability_Rand = 0;
    shapeEst = 1.024473 + 0.0178712*FE;
end
if (scaleEst < 0)
    Anisotropy_Spread_FE_Variability_Rand = 0;
    scaleEst = exp(-3.270029 + 0.3448803*log(FE));
end
...
% model: change with FE
NN_Shape_FE_Variability_Rand = randn(1)*NN_Shape_FE_Variability;
shapeEst = exp(-0.130851 + 0.4763741*log(FE)) + NN_Shape_FE_Variability_Rand;

if (shapeEst < 0)
    NN_Shape_FE_Variability_Rand = 0;
    shapeEst = exp(-0.130851 + 0.4763741*log(FE));
end
...
for i = 1:length(FE)
        ...
        patientInfo(i).Anisotropy_Shape_FE_Variability = Anisotropy_Shape_FE_Variability_Rand;
        patientInfo(i).Size_Spread_FE_Variability = Size_Spread_FE_Variability_Rand;
        patientInfo(i).Anisotropy_Spread_FE_Variability = Anisotropy_Spread_FE_Variability_Rand;
        patientInfo(i).NN_Shape_FE_Variability = NN_Shape_FE_Variability_Rand;
...
elseif cellBiasFlag == 3    
    switch distributionVariabilityType        
        ...
        case 2
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution3/Random'];                                          
        case 3
...
elseif cellBiasFlag == 4
    switch distributionVariabilityType        
        ...
        case 2
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution4/Random'];                                                      
        case 3
...
elseif cellBiasFlag == 5
    switch distributionVariabilityType        
        case 1
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/Mean'];  
        case 2
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/Random'];                                                      
        case 3
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/Size/Spread/' num2str(Size_Spread_FE_Variability)];
        case 4
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/NN/Shape/' num2str(NN_Shape_FE_Variability)];
        case 5  
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/Anisotropy/Spread/' num2str(Anisotropy_Spread_FE_Variability)];
        case 6    
            patientDir = [resultsDir '/FE-' num2str(FE) '/BiasedDistribution5/Anisotropy/Shape/' num2str(Anisotropy_Shape_FE_Variability)];
    end 
