...
%for patientIndx = selectedPatientIndx %1:length(patientInfo)
    ...
    %% All fields should be generated at 1T so we simply change the B0 value to change the scale of the field
    delBzGridP = delBzGridP*(B0*delX_multiplier/wetToDryWtRatio_multiplier);
    
    MriSimBlochP

    %% parameters for saving results
    if 0  %code chunk to be removed by Eamon, moving saving elsewhere
