function [cellSpheresSinusoids,cellSpheres,cellInfo,xTemp,yTemp,zTemp,cellIndx,cellFillCounter,R_select_orig,R_select,nnDist,nnPDF,nnCDF] = placeSpheresM10P2(NNfactor,NNfactorS,patientInfo,patientIndx,xTemp,yTemp,zTemp,sim_box_side,R_select,hepatocyte_dim)
...
%%%%% Send to cluster %%%%%%%%%%
jm = findResource('scheduler','type','jobmanager','name','MRISIM-JM','LookupURL','cluster1');
job = createJob(jm);
set(job, 'MaximumNumberOfWorkers', numberOfSinusoids);
set(job,'FileDependencies',{'placeSinusoidalSpheresM10P.m'});

for sinusoidCount = 1:numberOfSinusoids

    createTask(job,@placeSinusoidalSpheresM10P,3,{sinusoidCount,numSpheresSinusoidEach,xTemp,yTemp,zTemp,R_select,cylinderDiameter,nnCDF,nnDist,NNfactorS});
...
%%% filtering the spilt spheres, these are the ones which have a position
%%% of (0,0,0).

R_select_temp = R_select_S;
R_select_S = R_select_temp(xTempS~=0);
R_select_H = R_select_temp(xTempS==0);
clear R_select_temp;

xTempS_temp = xTempS;
xTempS = xTempS_temp(xTempS_temp~=0);
yTempS = yTempS(xTempS_temp~=0);
zTempS = zTempS(xTempS_temp~=0);
clear xTempS_temp;

numSpheresSinusoid = length(R_select_S);
numSpheresHepatocyte = length(R_select_H);
...
R_select_H = R_select_H(1:numSpheresHepatocyte);
